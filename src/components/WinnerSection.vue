  ...
  <section class="px-8 md:px-16 lg:px-24">
    <div v-for="movie in filteredMovies" :key="movie.sys.id" class="mb-8 md:mb-16">
      <div class="text-main text-3xl md:text-4xl lg:text-5xl font-bold mb-4 md:mb-8 lg:mb-16">
        <h2 class="dark:text-main">Winner {{ movie.fields.competitionYear }}</h2>
      </div>
      <div class="md:flex">
        <img :src="movie.fields.poster.fields.file.url" alt="" class="mb-4 md:w-1/2 lg:w-1/3">
        <div class="mb-4 md:w-1/2 lg:w-2/3 md:ml-16 dark:text-white">
    ...
    <p class="flex justify-center mb-8 md:text-2xl underline text-main cursor-pointer dark:text-main" @click="toggleAdditionalEntries">{{ showAdditionalWinners ? 'Hide past editions winners' : 'See past editions winners' }}</p>
  </section>
</template>
