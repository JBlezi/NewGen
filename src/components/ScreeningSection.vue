<template>
  <section>
    <div class="bg-cover" :style="{ backgroundColor: bgColor }">
      <div class="relative">
        <div class="flex">
          <div class="mt-8 md:my-16 flex flex-col px-8 md:px-16 h-full w-full">
            ...
            <font-awesome-icon v-if="movieList.movies.length > 2" :icon="['fas', 'chevron-right']" class="h-10 w-10 absolute bottom-1/2 right-5 transform translate-y-1/2 hidden md:block" :class="{'opacity-50': isRightArrowDisabledTablet}" @click="handleGalleryRightTablet"/>
            <font-awesome-icon v-if="movieList.movies.length > 2" :icon="['fas', 'chevron-left']" class="h-10 w-10 absolute bottom-1/2 left-5 transform translate-y-1/2 hidden md:block" :class="{'opacity-50': isLeftArrowDisabled}" @click="handleGalleryLeftTablet"/>
            <div class="md:flex md:justify-around md:mt-8 md:-mx-8">
              <div class="h-4/5">
